...
def fit(name):
    ...
    popt, pcov = curve_fit(func, data[:, 0], data[:, 1], maxfev=50000, p0=(3, 2, -1000, 1000))
...
def fit2(name, i1):
    data=np.loadtxt('sum-'+name+'.dat',  dtype=np.float,  delimiter=None)
    popt, pcov = curve_fit(func1, data[:, 0], data[:, 1], maxfev=50000, p0=(-1, 9, -3000, 3000))
    out=np.zeros(shape=(100,3))
    out[:, 2]=np.arange(101,2101,20)
